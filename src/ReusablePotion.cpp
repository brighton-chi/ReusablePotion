...
namespace
{
    std::unordered_map<ObjectGuid, SpellInfo const*> playerPotionSpell;
}
...
void SetPlayerPvPState(Player* player, bool state)
{
    ...
    if (mapKey != playerPvpMap.end())
        ...
        playerPvpMap.try_emplace(playerGuid, state);
}
...
void ReusablePotionPlayerScript::OnPlayerSpellCast(Player* player, Spell* spell, bool /*skipCheck*/)
{
    ...
    ObjectGuid guid = player ? player->GetGUID() : ObjectGuid::Empty;

    auto it = playerPotionSpell.find(guid);
    if (it != playerPotionSpell.end() && it->second)
    {
        if (player->IsInWorld() && player->HasSpell(it->second->Id))
        {
            player->SendCooldownEvent(it->second);
            player->SetLastPotionId(0);
        }
        playerPotionSpell.erase(it);
    }
    ...
    playerPotionSpell[guid] = spellInfo;
}
